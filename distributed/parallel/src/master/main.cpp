...
int main(int argc, char **argv)
{
    ...
    JobManager* jobManager = new JobManager(min_workers, pageSize);

    const int clusterSize = jobManager->ClusterSize();
    string outputDir = "./OUTPUT/" + string(argv[2]) + "/" + to_string(clusterSize) + "/" + string(argv[1]);
    ...
    if (op == 'E')
    {
        logFile << "ENC," << string(argv[1]) << "," << opEnd - opStart << "," << pageSize << "," << clusterSize << "," << CHUNK_SIZE / (1024 * 1024) << endl;
    }
    else
    {
        logFile << "DEC," << string(argv[1]) << "," << opEnd - opStart << "," << pageSize << "," << clusterSize << "," << CHUNK_SIZE / (1024 * 1024) << endl;
    }
    logFile.close();
